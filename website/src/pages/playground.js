...
export default function Playground() {
  ...
  return (
    <Layout
      title="Playground"
      description="Live in-browser react-x11 demos: edit real JSX and watch it render into a JavaScript X server on this page."
    >
      <main className="container margin-vert--lg">
        <Heading as="h1">Playground</Heading>
        <p className={styles.intro}>
          Everything on this page runs in your browser. The panel on the right
          is a real X session — a pure-JavaScript X server (with the RENDER
          extension react-x11 draws through) composited to a canvas — and the
          editor holds ordinary react-x11 code, the same code you would run in
          node against a real display. It connects through a custom{' '}
          <code>DISPLAY</code> transport (<code>demo/local:0</code>), lays out
          with the same yoga WASM, and shapes text with bundled DejaVu faces
          standing in for fontconfig. Edit it, press Run, and press{' '}
          <strong>Share</strong> to get a link that carries the whole snippet in
          its query string — there is no server storing any of this.
        </p>
        <div className={styles.perfNote}>
          <strong>This is the slow way to run react-x11.</strong> Everything
          here is emulation: the X server is JavaScript, rasterizing into a
          canvas on the same main thread as the editor, and input reaches it by{' '}
          <code>postMessage</code> across an iframe boundary. On a desktop the
          same code talks over a unix socket to a real X server — C, often
          GPU-accelerated — which does the compositing and glyph drawing itself.
          Judge the API here; judge the performance there.
        </div>
        {shared?.error && (
