 ...
 * backend, opaque on X11 (docs/elements.md), and not drawn at all on
 * XQuartz; `useSupports('glOverlay')` asks whether they will be. The
 * pointer over it is the tree's, as over any node: the handlers it inherits
 * (`onMouseDown`, `onClick`, `onWheel`, …) fire at the child under the
 * pointer or the `<glarea>` and bubble, on both backends, and
 * `GlAreaNode.forwardsPointer` says so at run time.
 */
