...
| `GetInputFocus`                                     | 4     | ntk's **frame fence**: the cheapest request with a reply, sent after a frame so its reply proves the server consumed the frame (`Window#_armFence`). See §4 — on ntk ≥ 8 with a Present-capable server the fence is replaced by the Present completion and does not appear |
...
The original capture shows both a per-frame fence round trip (20.5 ms RTT on
one frame — that RTT is the server draining the frame's mask ceremony, which
finding 1 removes) and Present blits. Current ntk clocks frames on
`Present:CompleteNotify` when the extension delivers it, and arms the fence
only as a fallback (`_clockOnPresent`); a live probe on this machine's
XQuartz confirms `frameClock: 'present'`, completions arriving, and **no**
per-frame `GetInputFocus`. A session showing both is either an older ntk, a
window pinned to `frameClock: 'fence'`, or a server whose completions
stalled (the watchdog then latches the fence until they resume). No change
proposed: the fence _is_ the flow control everywhere Present can't be, and
