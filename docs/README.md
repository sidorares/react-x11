...
- [architecture/video.md](architecture/video.md) — the design record for
  ...
  measured end to end, the biplanar-surface trap that decides the Cocoa
  design, and why a `<video>` element is one element with two very
  different insides. Not built.
...
- [gl.md](gl.md) — the two 3D backends: `glPolicy`, what each can draw,
  `<shaderMaterial>` — GLSL with three.js's names — and `<effectComposer>`
  post-processing, both on the direct one
