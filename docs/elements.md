...
- **Position**: `position` (`relative`, `absolute`, `static`, `sticky`, or
  one registered with `registerPosition`), `top`, `right`, `bottom`,
  `left`, `start`, `end`. Under `sticky` the insets are how close to the
