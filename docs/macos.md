...
| **eyedropper** (`useEyedropper`)  | **no rung** — `supported` is false and `pickScreenColor()` rejects typed; the X11 rung needs a pointer grab and a root read, which the cocoa `X` stub has neither of | `NSColorSampler` — the system sampler, out of process, so no Screen Recording grant of our own | react-x11#517 → windowkit/appkit#46                             |
