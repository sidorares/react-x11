  ...
  render() {
    ...
    return React.createElement(
      'window',
      {
        ...
        onMouseOut: () => this.setState({ down: false }),
        onMouseOver: () => this.setState({ down: true }),
        width: this.state.w,
        height: 500,
        title: `test ${this.state.isOver}`,
        onResize: (ev) =>
          this.setState({ isOver: `${ev.x} ${ev.y} ${ev.width} ${ev.height}` }),
      },
      React.createElement(
        'window',
        {
          ...
          x: 50,
          y: 150,
          onMouseOver: () => this.setState({ isOver: 'yes' }),
          onMouseOut: () => this.setState({ isOver: 'no' }),
        },
        ...
        React.createElement(
          'window',
          {
            key: 2,
            title: 'child child2',
            name: 'childChildWindow2',
            width: 20,
            height: 20,
            x: 15,
            y: 15,
            onMouseMove: grad1('green'),
          },
          React.createElement('window', {
            key: 3,
            title: 'child child child 2',
            name: 'child childChildWindow2',
            width: 12,
            height: 12,
            x: 5,
            y: 5,
            onMouseMove: grad1('blue'),
          }),
        ),
      ),
    );
