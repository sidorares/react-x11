...
| [`configurator/`](configurator/index.jsx) | a configure-to-order store page, styled like a current marketing site: display type from fonts shipped in the repo (`loadFont`, identical on every machine), hairline cards with `boxShadow` depth, size-query responsiveness, a live price, and a `<glarea>` product shot the scroll position opens and turns — with a flat `<box>` laptop for machines whose GL has no shaders. Tested in `test/configurator.test.js` |
