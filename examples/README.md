...
| [`configurator/`](configurator/index.jsx) | a configure-to-order store page, styled like a current marketing site: display type from fonts shipped in the repo (`loadFont`, identical on every machine), hairline cards with `boxShadow` depth, a gradient product render that follows the finish, size-query responsiveness, and a live price. Tested in `test/configurator.test.js` |
