...
| [`fonts.jsx`](fonts.jsx)                   | a font explorer: what `fc-match` ranks for a query, the metrics react-x11 measures with, which characters a face is missing — and a specimen with a canvas gradient and a server-side blurred shadow. Tested in `test/fonts.test.js` |
