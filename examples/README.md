...
| [`badge.jsx`](badge.jsx)                         | what an app says to the launcher while nobody is looking: an unread count with `useBadge()`, a `demands_attention` bounce set only while the window is unfocused, a `useDockMenu()`, and `createRoot({ cocoa: { appName, activationPolicy } })`. Best on the cocoa backend (a real Dock); degrades and says so elsewhere. Tested in `test/badge-example.test.js`                                                                                                        |
| [`tray.jsx`](tray.jsx)                           | a status menu that lives in the menu bar, not a window: `useTray()` over `NSStatusItem` with an SF Symbol per state and a menu that changes it, paired with `activationPolicy: 'accessory'` for a Dock-less app. On X11 the tray is inert (#353) and the window is the app. Tested in `test/tray-example.test.js`                                                                                                                                                       |
