...
| [`form/`](form/)                                 | `<textinput>`, `Select`, `RadioGroup`, `Slider`, `Checkbox`, and a modal `Dialog` — and a macOS app bundle: `npm run examples:form:app` builds `Guestbook.app` from `form/Info.plist` and `form/icon.svg` ([make-app.sh](form/make-app.sh))                                                                                                                                                                                                                                                                                                            |
...
| [`monitor.jsx`](monitor.jsx)                     | a process monitor, and the answer to "one keystroke changes hundreds of things — how do I keep the field responsive?" Two elements of its own via `registerElement`. Tested in `test/monitor.test.js`                                                                                                                                                                                                                                                                                                                                                  |
| [`chat.jsx`](chat.jsx)                           | the same features doing their actual job: an IRC-shaped client where `<Activity>` is what keeps a draft and `useOptimistic` is what makes a send feel instant. Tested in `test/chat.test.js`                                                                                                                                                                                                                                                                                                                                                           |
| [`finder.jsx`](finder.jsx)                       | a file browser: drag files out and in, copy and paste them, a windowed 50,000-entry listing, and a terminal embedded in the window with `<foreign>`. Tested in `test/finder.test.js`                                                                                                                                                                                                                                                                                                                                                                   |
| [`settings.jsx`](settings.jsx)                   | a preferences window: search that finds settings rather than pages, values that can be invalid, a reset that knows the default, and a language picker that mirrors the whole window. Tested in `test/settings.test.js`                                                                                                                                                                                                                                                                                                                                 |
| [`timer.jsx`](timer.jsx)                         | everything an app does besides draw: a single instance, a `com.example.x11timer://` deep link, a desktop notification, keep-awake, and a compatibility ladder for the machine with none of them. Tested in `test/timer.test.js`                                                                                                                                                                                                                                                                                                                        |
| [`badge.jsx`](badge.jsx)                         | what an app says to the launcher while nobody is looking: an unread count with `useBadge()`, a `demands_attention` bounce set only while the window is unfocused, a `useLauncherMenu()` — the Dock's menu on macOS and the launcher's quicklist on Linux — and `createRoot({ cocoa: { appName, activationPolicy } })`. Tested in `test/badge-example.test.js`                                                                                                                                                                                          |
| [`tray.jsx`](tray.jsx)                           | a status menu that lives in the tray, not a window: `useTray()` over `NSStatusItem` or `org.kde.StatusNotifierItem`, an icon per state and a menu that changes it, paired with `activationPolicy: 'accessory'` for a Dock-less app. Where no panel is hosting a tray, `available` stays false and the window is the app. Tested in `test/tray-example.test.js`                                                                                                                                                                                         |
| [`notify.jsx`](notify.jsx)                       | a download manager built around the part of a notification that is easy to get wrong: **one** banner updated in place while a transfer runs (`handle.update()`), `actions` + `onAction` landing back in React state, and `onClose(reason)`. Asks `useNotifier().backend` first and stays quiet on a rung that can neither update nor report. Tested in `test/notify-example.test.js`                                                                                                                                                                   |
| [`desktop.jsx`](desktop.jsx)                     | the whole desktop surface in one app, and the pane that says which of it this machine has: a backup agent whose run reaches the tray icon (`useTray` with `attention`, an overlay, a scroll and `error`), the launcher (`useBadge`, `useProgress`, `useLauncherMenu`, `setUrgent`) and a banner whose buttons are offered only where the daemon has `actions` — each chosen by a **feature** rather than by a platform, with `useDesktopCapability()` across `CAPABILITIES` rendered beside the live answers. Tested in `test/desktop-example.test.js` |
| [`fonts.jsx`](fonts.jsx)                         | a font explorer: what `fc-match` ranks for a query, the metrics react-x11 measures with drawn as guides, per-glyph shaping under the pointer, and a specimen with gradient ink and a server-side blurred shadow. Tested in `test/font-explorer.test.js`                                                                                                                                                                                                                                                                                                |
| [`configurator/`](configurator/index.jsx)        | a configure-to-order store page, styled like a current marketing site: display type from OFL fonts pulled in as devDependencies and loaded by path (`loadFont`, identical on every machine, nothing in git), hairline cards with `boxShadow` depth, size-query responsiveness, a live price, and a `<glarea>` product shot the scroll position opens and turns — with a flat `<box>` laptop for machines whose GL has no shaders. Tested in `test/configurator.test.js`                                                                                |
| [`viewer3d.jsx`](viewer3d.jsx)                   | a model viewer on raw `<glarea>`: indirect GLX, where geometry belongs in a display list and a frame is two matrices and one `CallList`. Tested in `test/viewer3d.test.js`                                                                                                                                                                                                                                                                                                                                                                             |
| [`foreign.jsx`](foreign.jsx)                     | `<foreign>`: another application's window in the layout — spawn a program into a pane, then let go of it                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [`selection.jsx`](selection.jsx)                 | `<box selectable>`: selecting text across blocks, and the PRIMARY selection no headless test can reach                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| [`container-queries.jsx`](container-queries.jsx) | container queries: one `<Card>` adapts to the pane it is in, a named container is reached past a nearer one. `npm run examples:container-queries`                                                                                                                                                                                                                                                                                                                                                                                                      |
| [`schedule.jsx`](schedule.jsx)                   | a conference schedule built on `position: 'sticky'`: each day's banner held at the top until the next day pushes it off, the room names held under it, the times held at the left, and the corner where the two meet — scroll it both ways and star the talks you want. `npm run examples:schedule`. Tested in `test/schedule-example.test.js`                                                                                                                                                                                                         |
| [`custom-layout.jsx`](custom-layout.jsx)         | a pinboard arranged four ways: the built-in `masonry`, a `justified` gallery registered in the file with `registerLayout`, `display: 'grid'` with CSS's own track lists, and flex-wrap for comparison — feature a pin and the rest pack around it; the toolbar's segmented controls are `equal-row`. `npm run examples:custom-layout`. Tested in `test/custom-layout-example.test.js`                                                                                                                                                                  |
...
| [`animation.jsx`](animation.jsx)                 | `transition` and `animation` side by side: cards that answer the pointer and reverse mid-flight, loops on colour, border and radius beside loops on position, the four easings on a track each, reduce motion live — and a frame counter in the terminal that reads 0 on the macOS layer presenter while the plain-box loops run in the render server. Tested in `test/animation-example.test.js`                                                                                                                                                      |
