...
| [`badge.jsx`](badge.jsx)                         | what an app says to the launcher while nobody is looking: an unread count with `useBadge()`, a `demands_attention` bounce set only while the window is unfocused, a `useDockMenu()` — the Dock's menu on macOS and the launcher's quicklist on Linux — and `createRoot({ cocoa: { appName, activationPolicy } })`. Tested in `test/badge-example.test.js`                                                                                                                                                                                          |
...
| [`desktop.jsx`](desktop.jsx)                     | the whole desktop surface in one app, and the pane that says which of it this machine has: a backup agent whose run reaches the tray icon (`useTray` with `attention`, an overlay, a scroll and `error`), the launcher (`useBadge`, `useProgress`, `useDockMenu`, `setUrgent`) and a banner whose buttons are offered only where the daemon has `actions` — each chosen by a **feature** rather than by a platform, with `useDesktopCapability()` across `CAPABILITIES` rendered beside the live answers. Tested in `test/desktop-example.test.js` |
