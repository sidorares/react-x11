...
| [`fonts.jsx`](fonts.jsx)                   | a font explorer: what `fc-match` ranks for a query, the metrics react-x11 measures with drawn as guides, per-glyph shaping under the pointer, and a specimen with gradient ink and a server-side blurred shadow. Tested in `test/fonts.test.js` |
