...
| [`custom-layout.jsx`](custom-layout.jsx)         | a pinboard arranged three ways by the `layout` style: the built-in `masonry`, a `justified` photo gallery registered in the file with `registerLayout`, and plain flex-wrap for comparison — feature a pin and the rest pack around it, and the toolbar's segmented controls are `equal-row`. `npm run examples:custom-layout`. Tested in `test/custom-layout-example.test.js`                                                                                          |
