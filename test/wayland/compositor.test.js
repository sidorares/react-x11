...
test(
  'the registry: globals are listed and binding works',
  { skip: SKIP },
  async () => {
...
test(
  'a toplevel: created without awaiting, configured by the compositor, acked lazily',
  { skip: SKIP },
  async () => {
...
test(
  'a reconfigure changes the size and states and is acked with the next frame',
  { skip: SKIP },
  async () => {
    const win = WaylandWindow.createSync({
      conn,
...
test(
  'the frame clock: a frame request rides the next commit and comes back with a time',
  { skip: SKIP },
  async () => {
    const win = WaylandWindow.createSync({
      conn,
...
test('a popup: positioner, parent and grab', { skip: SKIP }, async () => {
...
test(
  'the seat: pointer events come out shaped, with X button numbers and a state mask',
  { skip: SKIP },
  async () => {
    const win = WaylandWindow.createSync({
      conn,
...
test(
  'resize edges and a move are requests naming the seat and serial',
  { skip: SKIP },
  async () => {
    const win = WaylandWindow.createSync({
      conn,
...
test('close is a request the client hears', { skip: SKIP }, async () => {
  const win = WaylandWindow.createSync({
    conn,
    compositor,
    wmBase,
